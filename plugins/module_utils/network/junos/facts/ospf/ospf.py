        ...
        if ospf.get("area"):
            ...
            for area in areas:
                ...
                for interface in interfaces:
                    ...
                    interface_dict["timers"]["hello_interval"] = interface.get("hello-interval")
                    interface_dict["timers"]["dead_interval"] = interface.get("dead-interval")
                    interface_dict["timers"]["retransmit_interval"] = interface.get("retransmit-interval")
                    interface_dict["timers"]["transit_delay"] = interface.get("transit-delay")
                    interface_dict["timers"]["poll_interval"] = interface.get("poll-interval")
                ...
                if area.get("area-range"):
                    rendered_area["area_range"] = area["area-range"].get("name")
                if area.get("stub"):
            ...
            if "no-rfc-1583" in ospf.keys():
                processes["rfc1583compatibility"] = False
            if ospf.get("spf-options"):
                processes["spf_options"] = {}
                processes["spf_options"]["delay"] = ospf["spf-options"].get("delay")
                processes["spf_options"]["holddown"] = ospf["spf-options"].get("holddown")
                processes["spf_options"]["rapid_runs"] = ospf["spf-options"].get("rapid-runs")
            processes["overload"] = ospf.get("overload")
            processes["preference"] = ospf.get("preference")
            processes["external_preference"] = ospf.get("external-preference")
            processes["prefix_export_limit"] = ospf.get("prefix-export-limit")
            processes["reference_bandwidth"] = ospf.get("reference-bandwidth")
            processes["areas"] = rendered_areas
            processes["router_id"] = self.router_id["router-id"]
