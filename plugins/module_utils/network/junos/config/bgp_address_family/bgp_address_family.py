...
class Bgp_address_family(ConfigBase):
    ...
    def get_bgp_address_family_facts(self, data=None):
        ...
        bgp_facts = facts["ansible_network_resources"].get("bgp_address_family")
        if not bgp_facts:
            return {}
        return bgp_facts
    ...
    def execute_module(self):
        ...
        if state == "gathered":
            ...
            result["parsed"] = self.get_bgp_address_family_facts(data=running_config)
    ...
    def _state_merged(self, want, have):
        ...
        if w_af_list:
            for waf in w_af_list:
                # Add the nlri node
                nlri_node = build_child_xml_node(
                    family_root, waf['afi']
                )
                ...
                for type in nlri_types:
                    ...
                    type_node = build_child_xml_node(
                        nlri_node, type['type']
                    )
                    ...
                    if "accepted_prefix_limit" in type.keys():
                        ...
                        if 'limit_threshold' in apl.keys():
                            td_node = build_child_xml_node(
                                apl_node, "teardown", apl.get('limit_threshold')
                            )
                        ...
                        if 'idle_timeout_value' in apl.keys():
                            it_node = build_child_xml_node(
                                td_node, "idle-timeout", apl.get('idle_timeout_value')
                            )
                    ...
                    if "add_path" in type.keys():
                        ...
                        if 'send' in ap.keys():
                            # add node for send
                            send = ap.get('send')
                            send_node = build_child_xml_node(
                                ap_node, 'send'
                            )
                            # add node for path_count
                            if 'path_count' in send.keys():
                                build_child_xml_node(
                                    send_node, "path-count", send.get('path_count')
                                )
                            # add node for include_backup_path
                            if 'include_backup_path' in send.keys():
                                build_child_xml_node(
                                    send_node, "include-backup-path", send.get('include_backup_path')
                                )
                            ...
                            if 'prefix_policy' in send.keys():
                                build_child_xml_node(
                                    send_node, "prefix-policy", send.get('prefix_policy')
                                )
                    ...
                    if "defer_initial_multipath_build" in type.keys():
                        ...
                        if dimb and 'maximum_delay' in dimb.keys():
                            build_child_xml_node(
                                dimb_node, 'maximum-delay', dimb.get('maximum_delay')
                            )
                    ...
                    if "delay_route_advertisements" in type.keys():
                        ...
                        if 'max_delay_route_age' in dra.keys() or 'max_delay_routing_uptime' in dra.keys():
                            ...
                            if 'max_delay_route_age' in dra.keys():
                                build_child_xml_node(
                                    maxd_node, 'route-age', dra.get('max_delay_route_age')
                                )

                            # add node routing-uptime
                            if 'max_delay_routing_uptime' in dra.keys():
                                build_child_xml_node(
                                    maxd_node, 'routing-uptime', dra.get('max_delay_routing_uptime')
                                )
                        ...
                        if 'min_delay_inbound_convergence' in dra.keys() or 'min_delay_routing_uptime' in dra.keys():
                            ...
                            if 'min_delay_inbound_convergence' in dra.keys():
                                build_child_xml_node(
                                    mind_node, 'inbound-convergence', dra.get('min_delay_inbound_convergence')
                                )

                            # add node routing-uptime
                            if 'min_delay_routing_uptime' in dra.keys():
                                build_child_xml_node(
                                    mind_node, 'routing-uptime', dra.get('min_delay_routing_uptime')
                                )
                    ...
                    if "local_ipv4_address" in type.keys():
                        # add node local-ipv4-address
                        build_child_xml_node(
                            type_node, 'local-ipv4-address', type.get('local_ipv4_address')
                        )
                    ...
                    if "output_queue_priority_expedited" in type.keys() or\
                            "output_queue_priority_priority" in type.keys():
                        # node for output-queue-priority
                        oqp_node = build_child_xml_node(
                            type_node, 'output-queue-priority'
                        )
                        # add node expedited
                        if "output_queue_priority_expedited" in type.keys() and\
                                type.get('output_queue_priority_expedited'):
                            build_child_xml_node(
                                oqp_node, 'expedited'
                            )
                        # add node priority
                        if "output_queue_priority_priority" in type.keys():
                            build_child_xml_node(
                                oqp_node, 'priority', type.get('output_queue_priority_priority')
                            )
                    ...
                    if "prefix_limit" in type.keys():
                        ...
                        if 'idle_timeout_value' in pl.keys():
                            it_node = build_child_xml_node(
                                td_node, "idle-timeout", pl.get('idle_timeout_value')
                            )
                    ...
                    if "resolve_vpn" in type.keys():
                        if type.get('resolve_vpn'):
                            build_child_xml_node(
                                type_node, 'resolve-vpn'
                            )

                    #  add rib
                    if "rib" in type.keys():
                        rib_node = build_child_xml_node(
                            type_node, 'rib'
                        )
                        # add node inet.3
                        build_child_xml_node(
                    ...
                    if "route_refresh_priority_expedited" in type.keys() or \
                            "route_refresh_priority_priority" in type.keys():
                        # node for route-refresh-priority
                        rrp_node = build_child_xml_node(
                            type_node, 'route-refresh-priority'
                        )
                        # add node expedited
                        if "route_refresh_priority_expedited" in type.keys() and \
                                type.get('route_refresh_priority_expedited'):
                            build_child_xml_node(
                                rrp_node, 'expedited'
                            )
                        # add node priority
                        if "route_refresh_priority_priority" in type.keys():
                            build_child_xml_node(
                                rrp_node, 'priority', type.get('route_refresh_priority_priority')
                            )
                    ...
                    if "withdraw_priority_expedited" in type.keys() or \
                            "withdraw_priority_priority" in type.keys():
                        # node for withdraw-priority
                        wp_node = build_child_xml_node(
                            type_node, 'withdraw-priority'
                        )
                        # add node expedited
                        if "withdraw_priority_expedited" in type.keys() and \
                                type.get('withdraw_priority_expedited'):
                            build_child_xml_node(
                                wp_node, 'expedited'
                            )
                        # add node priority
                        if "withdraw_priority_priority" in type.keys():
                            build_child_xml_node(
                                wp_node, 'priority', type.get('withdraw_priority_priority')
                            )
                    ...
                    if "strip_nexthop" in type.keys():
                        if type.get('strip_nexthop'):
                            build_child_xml_node(
                                type_node, 'strip-nexthop'
                            )

                    # add topology
                    if "topology" in type.keys():
                        top = type.get('topology')
                        top_node = build_child_xml_node(
                            type_node, 'topology'
                        )
                        if 'name' in top.keys():
                            build_child_xml_node(
